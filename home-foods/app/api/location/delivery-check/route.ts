...
import { getKitchenDeliveryDistances, isWithinDeliveryRadius, ROUTING_UNAVAILABLE_MESSAGE, verifyAddressText, verifyCoordinates } from "@/src/lib/location";
...
export async function POST(request: Request) {
  try {
    ...
    if (Number.isInteger(body.addressId) && Number(body.addressId) > 0) {
      ...
      const saved = await db.orm.public.Address.where({ id: Number(body.addressId), userId: session.userId }).first();
      if (!saved) return Response.json({ error: "That saved address isn't available on your account." }, { status: 404 });
      address = saved.latitude != null && saved.longitude != null ? await verifyCoordinates(saved.latitude, saved.longitude) : await verifyAddressText(`${saved.addressLine1}, ${saved.postalCode ?? ""} ${saved.city}, Finland`);
    ...
    const shops = await db.orm.public.Shop.where((shop) => shop.id.in(ids)).select("id", "name", "status", "latitude", "longitude").include("seller", (seller) => seller.select("name", "email")).all();
    ...
    const distances = isDeliveryRadiusEnforced() ? await getKitchenDeliveryDistances(address, locatedShops.map((shop) => ({ latitude: shop.latitude!, longitude: shop.longitude! }))) : [];
    ...
    const status = message.includes("not available in this country") ? 422 : message.includes("Geoapify isn't configured") || message === ROUTING_UNAVAILABLE_MESSAGE ? 503 : 400;
