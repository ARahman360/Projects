...
export async function GET() {
  ...
  try {
    const favorites = await db.orm.public.Favorite.where({ customerId: session.userId }).include("menuItem").include("shop", (shop) => shop.select("id", "name", "city")).orderBy((favorite) => favorite.createdAt.desc()).all();
    const favoriteKitchens = await db.orm.public.KitchenFavorite.where({ customerId: session.userId }).include("shop", (shop) => shop.select("id", "name", "city", "coverImageUrl", "logoUrl")).orderBy((favorite) => favorite.createdAt.desc()).all();
