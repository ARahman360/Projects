  ...
  try {
    ...
    await db.transaction(async tx => {
      if (["approve-kitchen", "reject-kitchen", "suspend-kitchen", "reactivate-kitchen", "edit-kitchen"].includes(action)) {
        ...
        if (!shop) throw new Error("Kitchen not found.");
        ...
        if (!statuses[action].includes(shop.status)) throw new Error("The kitchen status changed. Refresh before trying again.");
        ...
        if (status === "ACTIVE" && action !== "edit-kitchen") {
          ...
          if (seller?.accountStatus !== "ACTIVE" || !shop.address || !shop.city || shop.latitude == null || shop.longitude == null || !isKitchenLocationAllowed(shop.address)) throw new Error("An active seller and a registered permitted kitchen location are required before approval.");
        }
        ...
        const updated = await tx.orm.public.Shop.where({ id, status: shop.status }).update(changes);
        if (!updated) throw new Error("Kitchen changed. Refresh and retry.");
        ...
        if (!user || !["CUSTOMER", "RIDER"].includes(user.role)) throw new Error("Only customer and rider accounts can be managed here.");
        if (action === "verify-rider") {
          if (user.role !== "RIDER") throw new Error("This account is not a rider.");
      ...
      } else throw new Error("This administrative action is not supported.");
    });
    return Response.json({ success: true });
  } catch (error) { return jsonError(error instanceof Error ? error.message : "The action could not be saved.", 409); }
}
